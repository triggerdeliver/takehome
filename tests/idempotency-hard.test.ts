...
const INITIAL_EVENTS = 200;
const DUPLICATE_PERCENT = 20;
const DUPLICATE_DELAY_MS = 2000;

const redis = new Redis(REDIS_URL);
const kafka = new Kafka({ clientId: 'idempotency-hard-test', brokers: [KAFKA_BROKER] });
...
async function sendInitialEvents(count: number): Promise<SentEvent[]> {
  ...
  for (let i = 0; i < count; i += batchSize) {
    ...
    const ip = `10.5.0.${Math.floor(i / batchSize) % 250}`;
...
async function runIdempotencyTest(): Promise<TestResult> {
  console.log('\n' + '═'.repeat(60));
  console.log('IDEMPOTENCY HARD TEST V2.1');
...
function printResult(result: TestResult): void {
  console.log('\n' + '═'.repeat(60));
  console.log('IDEMPOTENCY HARD TEST V2.1 RESULTS');
  console.log('═'.repeat(60));
