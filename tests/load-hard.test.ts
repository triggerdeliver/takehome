...
const TOTAL_EVENTS = parseInt(process.env.TOTAL_EVENTS || '1000000', 10);
const TIME_LIMIT_MS = 30000;
const MIN_SUCCESS_RATE = 0.95;
const BATCH_SIZE = 1000;
const CONCURRENT_SENDERS = 80;
const SAMPLE_SIZE = 500;

const redis = new Redis(REDIS_URL);
const kafka = new Kafka({ clientId: 'load-test-v2', brokers: [KAFKA_BROKER] });
...
async function sendBatch(
  ...
  try {
    const response = await axios.post(
      ...
      { events },
      { timeout: 15000, headers: { 'x-forwarded-for': ip } }
    );
...
async function sendAllEvents(): Promise<{ totalSent: number; allEventIds: string[] }> {
  const userIds = Array.from({ length: 20000 }, (_, i) => `user-${i}`);
    ...
    promises.push(
      (async () => {
        ...
        for (const batch of chunk) {
          const octetA = Math.floor(ipCounter / 250);
          const octetB = ipCounter % 250;
          const ip = `10.0.${octetA}.${octetB}`;
          ipCounter += 1;
          const result = await sendBatch(batch, ip);
...
async function runLoadTest(): Promise<TestResult> {
  console.log('\n' + '═'.repeat(60));
  console.log('LOAD TEST V2.1: High throughput');
  ...
  console.log('\nWaiting for processing (checking Redis directly)...');
...
function printResult(result: TestResult): void {
  console.log('\n' + '═'.repeat(60));
  console.log('LOAD TEST V2.1 RESULTS');
  console.log('═'.repeat(60));
